...
const Index = () => {
  ...
  const gondElements = [<GondTree className="w-8 h-12" />, <GondPeacock className="w-12 h-8" />, <GondHut className="w-10 h-8" />];
  const skillsData = [{
      ...
      <main className="gond-bg">
        ...
        <section className="min-h-screen flex flex-col justify-center relative px-4 overflow-hidden">
          ...
          <div className="container mx-auto max-w-5xl pt-20 relative z-10">
            <span className={`font-medium mb-4 block transition-colors duration-500 ${theme === 'light' ? 'text-gond-light-orange' : 'text-gond-dark-orange'}`}>
              Visual Storyteller
            </span>
            
            <h1 className="text-4xl md:text-6xl lg:text-7xl font-bold mb-6 leading-tight gond-text">
              Bringing ideas to life through{" "}
              <span className={`transition-colors duration-500 ${theme === 'light' ? 'text-gond-light-purple' : 'text-gond-dark-purple'}`}>
                visual creativity
              </span>{" "}
              and artistry
            </h1>
            
            <p className="text-xl gond-text-light mb-8 max-w-2xl">I'm Yogesh Sharma, a passionate creative specializing in video editing, photo editing, and graphic design. Transforming concepts into compelling visual stories.</p>
            
            <div className="flex flex-wrap gap-4">
              <Button asChild size="lg" className="gond-button">
                <Link to="/portfolio">View My Work</Link>
              </Button>
              <Button variant="outline" size="lg" asChild className={`transition-all duration-300 border-2 ${theme === 'light' ? 'text-gond-light-orange border-gond-light-orange hover:bg-gond-light-orange hover:text-white' : 'text-gond-dark-orange border-gond-dark-orange hover:bg-gond-dark-orange hover:text-gond-dark-bg'}`}>
          ...
          <div className="absolute bottom-10 left-1/2 transform -translate-x-1/2 flex flex-col items-center z-10">
            <button onClick={scrollToAbout} className="gond-text-light hover:gond-text transition-all duration-300 flex flex-col items-center group">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center relative z-10">
              <div className="relative">
                ...
                <div className="space-y-4 gond-text-light">
                  <p>I'm a visual creative from Raipur, Chhattisgarh, India with over 5 years of experience in digital media production. My work spans photography, videography, and graphic design – all tied together by a passion for visual storytelling.</p>
                  <p>
              ...
              <div className="gond-card p-6 relative">
                ...
                <div className="space-y-4">
                  {skillsData.map((skill, index) => <div key={index}>
                      ...
                      <div className={`w-full rounded-full h-2 transition-colors duration-500 ${theme === 'light' ? 'bg-gond-light-cream' : 'bg-gond-dark-cream'}`}>
                        <div className={`h-2 rounded-full transition-all duration-1000 ease-out ${getSkillColor(skill.color)}`} style={{
        ...
        <section className="py-20 px-4 gond-bg">
          <div className="container mx-auto max-w-5xl relative">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 relative z-10">
              {(featuredProjects.length > 0 ? featuredProjects : placeholderProjects).map((project, index) => <div key={project.id} className="group portfolio-item gond-card relative overflow-hidden">
                  {project.category === 'videos' && isVideoFile(project.image) ? <div className="relative w-full h-72">
                      {project.thumbnail ? <>
                          <img src={project.thumbnail} alt={project.title} className="w-full h-72 object-cover transition-transform duration-500 group-hover:scale-105 group-hover:opacity-0" />
                          <video src={project.image} className="absolute inset-0 w-full h-72 object-cover transition-all duration-500 opacity-0 group-hover:opacity-100 group-hover:scale-105" muted loop playsInline onMouseEnter={e => e.currentTarget.play()} onMouseLeave={e => {
                    e.currentTarget.pause();
                    e.currentTarget.currentTime = 0;
                  }} />
                        </> : <video src={project.image} className="w-full h-72 object-cover transition-transform duration-500 group-hover:scale-105" muted loop playsInline onMouseEnter={e => e.currentTarget.play()} onMouseLeave={e => {
                  e.currentTarget.pause();
                  e.currentTarget.currentTime = 0;
                }} />}
                    </div> : <img src={project.image} alt={project.title} className="w-full h-72 object-cover transition-transform duration-500 group-hover:scale-105" />}
                  ...
                  <div className={`absolute top-2 right-2 opacity-40 transition-opacity duration-300 ${theme === 'dark' ? 'group-hover:opacity-80' : 'group-hover:opacity-60'}`}>
